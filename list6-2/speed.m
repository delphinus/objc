...
- (id)initWithNext: (id)obj {
    self = [super init];
    next = [obj retain];    // MRC では retain
    return self;
}

+ (Cell *)cellWithNext: (id)obj {
    return [[[self alloc] initWithNext: obj] autorelease];
} // MRC ではさらに autorelease してから返す
...
int main(void) {
    ...
    for (i = 0; i < LOOP; i++) {
        @autoreleasepool { // GC では自動開放プールを使わない
            for (j = 0; j < LOOP; j++) {
                int idx = random() & ARRAYMASK;
                if (buf[idx] != nil && (random() & ACCIDENT) == 0) {
                    // 時々、オブジェクトを解放する
                    [buf[idx] release]; // MRC のみ
                    buf[idx] = nil;
                } else {
                    // 通常は配列に線形リストを構成していく
                    id t = buf[idx];
                    buf[idx] = [[Cell cellWithNext: t] retain]; // MRC のみ
                    [t release]; // MRC のみ
                }
            }
        } // GC ではここでガーベジコレクションの実行を促す
    }
    return 0;
}
